...
import { PREVIOS_SESSION_CONTEXT, WORKSESSION_ID, WORKSESSION_SORTING_ID } from "@/utils/constants"
...
export default function Sorting() {
    ...
    const onSubmit = async (data: WorkInputFormValues) => {
        try {
            ...
            const response = await workSessionSortingServies.createWorkSessionSorting({
            ...
            if (response.id) {
                localStorageService.set<String>(WORKSESSION_SORTING_ID, response.id)
                router.push("/normal-production/sorting-progress")
            }
        } catch (error) {
