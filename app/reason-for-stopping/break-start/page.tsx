...
import { PREVIOS_SESSION_CONTEXT, REASON_FOR_STOPPING_BREAK_START, WORKSESSION_ID } from "@/utils/constants"
...
export default function BreakStart() {
    ...
    const onSubmit = async (data: WorkInputFormValues) => {
        try {
            ...
            const response = await reasonForStoppingBreakStartServies.createReasonForStoppingBreakStart({
            ...
            if (response.id) {
                localStorageService.set<String>(REASON_FOR_STOPPING_BREAK_START, response.id)
