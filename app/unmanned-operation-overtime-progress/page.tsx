...
import { WORKSESSION_UNMANNED_OPERATION_OVER_TIME_ID } from "@/utils/constants"
...
export default function UnmannedOperationOvertimeProgress() {
    ...
    const unmannedOperationOvertimeId = localStorageService.get<string>(WORKSESSION_UNMANNED_OPERATION_OVER_TIME_ID, '');
    ...
    const getWorkSessionUnmannedOperationOvertimeId = useCallback(async () => {

        await unmannedOperationOvertimesServies.getUnmannedOperationOvertimesId(unmannedOperationOvertimeId).then((res) => {
            handleSetValueDefault(res.unmannedOperationOvertime)

        }).catch((error) => { })

    }, [])

    const handleSetValueDefault = (data: UnmannedOperationOvertime) => {
    ...
    const handleEndUnmanned = async () => {
        ...
        if (formData.remark) {
            await unmannedOperationOvertimesServies.updateUnmannedOperationOvertimesRemark(unmannedOperationOvertimeId, formData.remark);
        }

        await unmannedOperationOvertimesServies.endUnmannedOperationOvertimes({
            id: unmannedOperationOvertimeId,
    ...
    const handleUpdateNumberOfGoodProducts = async (value: string) => {
        try {
            const response = await unmannedOperationOvertimesServies.updateNumberOfGoodProducts(unmannedOperationOvertimeId, parseInt(value));
            if (response.id) {
    ...
    const handleUpdateCanNo = async (value: string) => {
        try {
            const response = await unmannedOperationOvertimesServies.updateCanNo(unmannedOperationOvertimeId, parseInt(value));
            if (response.id) {
    ...
    const handleUpdateUnmannedTime = async (value: string) => {
        try {
            const response = await unmannedOperationOvertimesServies.updateUnmannedTime(unmannedOperationOvertimeId, parseInt(value));
            if (response.id) {
    ...
    useEffect(() => { getWorkSessionUnmannedOperationOvertimeId() }, [getWorkSessionUnmannedOperationOvertimeId])
            ...
            <div className="max-w-7xl mx-auto bg-sky-100 p-6 rounded-md">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex flex-col space-y-6 mt-[50%]">
                        <TimerDisplay
                            timerId="unmanned-operation-timer"
                            autoStart={true}
